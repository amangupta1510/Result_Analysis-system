...
<div class="limiter">
    <div class="title">
        <div class="searchBox">
            <h4><b>{{ app('request')->input('fname')}} Results</b>
                {{-- <div class="searchForm">
                    <form action="{{route('admin-folder_test_list_result')}}" method="get">
                        <input id="searchField" type="text" name="s" value="{{ app('request')->input('s')}}" placeholder="Search here" />
                        <input type="hidden" name="flid" value="{{ app('request')->input('flid')}}">
                        <input type="hidden" name="fname" value="{{ app('request')->input('fname')}}">
                        <div class="close">
    ...
    $(document).on('click', '.pagination a', function(e) {
        ...
        var img = "{{ route('admin-folder_test_list_result_page',['flid'=>app('request')->input('flid'),'fname'=>app('request')->input('fname'),'page'=>':page']) }}";

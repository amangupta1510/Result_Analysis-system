...
<div id="show" class="modal fade " role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title paper_title"></h4>
            </div>
...
<div class="limiter">
        ...
        <div class="searchBox">
            <h4><b>Results</b>
                <div class="searchForm">
                    <form action="{{route('admin-student_result')}}" method="get">
                    ...
                    <tbody>
                        ...
                        <tr class="user{{$user->id}}">
                            ...
                            <td class="column6" style="padding: 5px 5px 5px 5px;cursor:pointer;">@if($user->rank!='')
                                <a>AIR {{$user->rank}} &nbsp;</a>@endif<strong><a href="{{ route('admin-result_analysis',['id' =>$user->id]) }}">RESULT</a></strong>&nbsp;&nbsp;<a id="show-button" title="Show Responses" class="btn btn-success" style="color: #fff;zoom:.6" data-toggle="modal" data-target="#show" data-id="{{$user->pid}}" data-plid="{{$user->plid}}" data-type="{{$user->type}}" data-paper="{{$user->paper}}" data-sid="{{$user->sid}}"><i class="fa fa-eye" style="color: #fff"></i></a></td>
    ...
    $(document).on('click', '.pagination a', function(e) {
        ...
        var img = "{{ route('admin-student_result_page',['id'=>':id','s'=>':year','page'=>':page']) }}";
